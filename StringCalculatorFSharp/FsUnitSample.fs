...
let add(x:string) = 
    let delimeter = if(x.StartsWith("//")) then x.Substring(3, 1) else ""
    let regex = Regex("[\n,"+delimeter+"]")
    let cleanStr=x.Replace("//","")
    System.Console.WriteLine(cleanStr)

    if (cleanStr="") then 0
    else add_rec(regex.Split(cleanStr)|>Seq.toList, 0)
